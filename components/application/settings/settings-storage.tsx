...
function SortableHeader({
  ...
  return (
    <button
      ...
      className="focus-visible:ring-border-focus-ring flex cursor-pointer items-center gap-0.5 rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
    >
      <span
        className={cx(
      ...
      <span className="flex size-6 shrink-0 items-center justify-center">
        <ChevronSortDown
          className={cx(
            'ease size-6 transition-[transform,color] duration-150',
    ...
    <Dropdown isOpen={isOpen} onOpenChange={setIsOpen}>
      <TooltipTrigger delay={200}>
        <DropdownTrigger
          ...
          className={cx(
            'rounded-2lg relative inline-flex size-8 shrink-0 items-center justify-center',
            'border-border-button-default bg-background-primary-default text-foreground-icon-primary border shadow-xs',
            'ease transition-[background-color,border-color,box-shadow,color] duration-150',
        ...
        <DropdownGroup>
          {MORE_MENU_ACTIONS.map(({ icon: Icon, label }) => (
            <DropdownItem key={label} onSelect={() => setIsOpen(false)} className="px-2 py-1.5">
              <Icon className="text-foreground-icon-secondary size-[18px] shrink-0" aria-hidden />
...
function AnimatedRow({ phase, children }: { phase: RowAnimationPhase; children: ReactNode }) {
  return (
    <div
      className={cx(
        'grid grid-rows-[1fr]',
        phase === 'entering' && 'animate-row-grow-in',
        phase === 'exiting' && 'animate-row-collapse-out pointer-events-none',
      )}
    >
...
export function SettingsStorage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort) {
      ...
      rows.sort((a, b) =>
        recency === 'newest'
          ? b.uploadedStamp - a.uploadedStamp
          : a.uploadedStamp - b.uploadedStamp,
      )
    }
    return rows
  ...
  return (
    ...
    <div className="flex w-full flex-col gap-6 pt-2.5">
      ...
      <section className="border-border-table flex w-full flex-col rounded-2xl border pt-2 pb-3">
        {/* Toolbar */}
        <div className="flex w-full flex-col items-start gap-3 px-3 py-1 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col justify-center">
            <p className="text-body-medium text-text-tertiary whitespace-nowrap">Stored in</p>
            <p className="text-body-medium text-text-primary whitespace-nowrap">
        ...
        {/* Column headers */}
        <div className="border-separator-border bg-background-secondary-default mt-2 flex w-full items-center border-y pl-3">
          ...
          <div className="flex w-[116px] shrink-0 items-center px-3 py-2.5">
            <span className="text-body-medium text-text-tertiary whitespace-nowrap">Actions</span>
          </div>
        </div>
            ...
            rows.map(file => {
              ...
              return (
                <AnimatedRow key={file.id} phase={phase}>
                  <div className="border-separator-border flex w-full items-center border-b">
                    <div className="flex min-w-0 flex-1 items-center gap-2 py-2.5">
                      <Checkbox
