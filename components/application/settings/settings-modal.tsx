    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4" role="presentation">
      ...
      <div
        className={cx(
          ...
          visible ? 'blur-0 scale-100 opacity-100' : 'scale-[0.85] opacity-0 blur-[4px]',
        )}
      >
        {/* Panel */}
        <div
          ...
          className={cx(
            ...
            'bg-background-full overflow-clip rounded-3xl shadow-xs outline-none',
          )}
        >
          {/* Nav rail — the board-team dropdown group/item recipe */}
          <nav
            aria-label="Settings sections"
            className="border-separator-border bg-background-secondary-default flex w-[274px] shrink-0 flex-col gap-5 overflow-y-auto border-r p-2.5"
          >
            {NAV_GROUPS.map(group => (
              <div key={group.label} className="flex w-full flex-col gap-1.5 pt-1">
                <span className="text-body-medium text-text-secondary pl-2">{group.label}</span>
                  ...
                  {group.items.map(item => {
                    ...
                    return (
                      <button
                        ...
                        className={cx(
                          'rounded-2lg flex w-full cursor-pointer items-center gap-2 p-2 text-left',
                          'ease focus-visible:ring-border-focus-ring transition-colors duration-150 outline-none focus-visible:ring-2',
                          selected
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            >
              ...
              <button
                ...
                className={cx(
                  ...
                  'ease hover:bg-background-tertiary-hover transition-colors duration-150',
            ...
            <div className="relative min-h-0 flex-1">
              ...
              <div
                aria-hidden
                className={cx(
                  'from-background-primary-default pointer-events-none absolute inset-x-0 top-0 h-10 bg-linear-to-b to-transparent',
        ...
        <div
          ...
          className={cx(
            ...
            'border-border-button-default bg-background-primary-default shadow-dropdown rounded-full border py-1 pr-2.5 pl-1.5',
            'transition-[opacity,transform,filter] duration-200 ease-out',
            savedPhase === 'shown' && 'blur-0 translate-y-1/2 scale-100 opacity-100',
            // Starts below the resting spot, rises in; keeps drifting up on exit.
            savedPhase === 'hidden' && 'translate-y-[calc(50%+12px)] scale-90 opacity-0 blur-[2px]',
            savedPhase === 'leaving' &&
              'translate-y-[calc(50%-10px)] scale-90 opacity-0 blur-[2px]',
          )}
        >
          <RiCheckboxCircleFill className="size-4 shrink-0 text-lime-600" aria-hidden />
          <span className="text-body-2-medium text-text-primary whitespace-nowrap">Saved</span>
        </div>
      </div>
    </div>,
    document.body,
