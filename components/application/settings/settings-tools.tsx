...
function ServerTile({ server }: { server: McpServer }) {
  return (
    <span className="relative flex size-8 shrink-0 items-center justify-center rounded-lg">
      <span
        className={cx(
          'text-body-2-medium flex size-8 items-center justify-center rounded-lg',
          server.tileClass,
        )}
      >
        {server.initial}
      ...
      <span
        aria-hidden
        className={cx(
          'ring-background-secondary-default absolute -bottom-0.5 -left-0.5 size-2.5 rounded-full ring-2',
    ...
    <Dropdown isOpen={isOpen} onOpenChange={setIsOpen}>
      <DropdownTrigger
        ...
        className={cx(
          ...
          'text-foreground-icon-secondary ease transition-colors duration-150 outline-none',
          'hover:bg-background-secondary-hover focus-visible:ring-border-focus-ring focus-visible:ring-2',
        ...
        <DropdownGroup>
          {['Show output', 'Refresh tools', 'Remove server'].map(label => (
            <DropdownItem key={label} onSelect={() => setIsOpen(false)} className="px-2 py-1.5">
              <span className="text-body-medium text-text-primary truncate whitespace-nowrap">
                {label}
              </span>
            </DropdownItem>
          ))}
...
function InlineAction({ children, onClick }: { children: ReactNode; onClick?: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cx(
        'text-body-2-regular text-text-tertiary cursor-pointer rounded-sm whitespace-nowrap',
        'ease transition-colors duration-150 outline-none',
        'hover:text-text-secondary focus-visible:ring-border-focus-ring focus-visible:ring-2',
      )}
    >
      {children}
    </button>
...
function ServerRow({ server }: { server: McpServer }) {
  ...
  return (
    <div className="border-separator-border border-b py-2.5 pr-2.5 last:border-b-0">
      <div className="flex w-full items-center gap-2.5">
        <ServerTile server={server} />
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex items-baseline gap-2">
            <p className="text-body-medium text-text-primary whitespace-nowrap">{server.name}</p>
            <InlineAction>Logout</InlineAction>
          </div>
          {server.status === 'connected' ? (
            <div className="flex items-center gap-1">
              <p className="text-body-2-regular text-text-secondary truncate">{server.summary}</p>
              {server.tools && (
                <button
                  type="button"
                  ...
                  className={cx(
                    ...
                    'text-foreground-icon-tertiary ease transition-colors duration-150 outline-none',
                    'hover:text-foreground-icon-secondary focus-visible:ring-border-focus-ring focus-visible:ring-2',
                  )}
                >
            ...
            <div className="flex items-center gap-1">
              <p className="text-body-2-regular text-text-secondary whitespace-nowrap">Error</p>
  ...
  return (
    <button
      ...
      className={cx(
        ...
        'focus-visible:ring-border-focus-ring outline-none focus-visible:ring-2 focus-visible:ring-inset',
      )}
    >
      <span
        className={cx(
          'bg-background-tertiary-default flex size-8 shrink-0 items-center justify-center rounded-lg',
          'ease group-hover:bg-background-tertiary-hover transition-colors duration-150',
        )}
      >
        <RiAddLine className="text-foreground-icon-secondary size-5 shrink-0" aria-hidden />
      </span>
      <span className="flex min-w-0 flex-col">
        <span className="text-body-medium text-text-primary whitespace-nowrap">New MCP Server</span>
        <span className="text-body-2-regular text-text-secondary whitespace-nowrap">
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      >
        {SCOPES.map(s => (
          <PillTab
            key={s.id}
            variant="gray"
            isSelected={s.id === scopeId}
            onSelect={() => setScopeId(s.id)}
          >
            {s.label}
          </PillTab>
        ))}
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <SettingsCard>
          ...
          >
            <Switch
              isSelected={waitForAuth}
              onChange={setWaitForAuth}
              aria-label="Wait for MCP authentication"
            />
          </SettingsRow>
        </SettingsCard>
      </div>
      ...
      <div className="flex w-full flex-col gap-2">
        <div className="flex w-full flex-col gap-0.5">
          ...
          <p className="text-body-2-regular text-text-tertiary w-full px-2">
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <div className="bg-background-secondary-default flex w-full flex-col items-center gap-3 rounded-2xl px-6 py-8">
          <div className="flex flex-col items-center gap-1 text-center">
            <p className="text-body-medium text-text-primary">No Team MCP Servers</p>
            <p className="text-body-2-regular text-text-secondary max-w-[360px]">
