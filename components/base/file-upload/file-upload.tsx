...
export function FileUpload({
  ...
  const startUpload = (nextFile: File) => {
    ...
    if (!allowedExtensions.map(value => value.toLowerCase()).includes(extension)) {
      setRejection(
        `Only ${allowedExtensions.map(value => value.toUpperCase()).join(', ')} files are supported`,
      )
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className={cx(
          'absolute -top-[9.5px] left-1/2 -translate-x-1/2 rounded-md bg-blue-400 px-1.5 py-0.5',
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          >
            {rejection ?? (
              <>
                Drag and drop to upload or <span className="text-blue-500">select</span>
              </>
            )}
          </p>
