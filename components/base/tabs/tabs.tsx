  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <AriaTabList
        ...
        className={state =>
          cx(
            'border-separator-border flex w-full items-center gap-1 border-b',
            typeof className === 'function' ? className(state) : className,
          )
        }
      />
      {underline && (
        <span
          aria-hidden
          className="ease pointer-events-none absolute bottom-0 left-0 h-0.5 bg-blue-600 transition-[transform,width] duration-200"
          style={{
        ...
        <>
          <span
            className={cx(
              ...
              isSelected ? 'text-body-medium text-blue-600' : 'text-body-regular text-text-primary',
            )}
          >
          ...
          {count != null && (
            <span
              className={cx(
                ...
                isSelected
                  ? 'bg-tab-count-selected-background text-blue-600'
...
export function TabPanel({ className, ref, ...props }: TabPanelProps) {
  return (
    <AriaTabPanel
      ...
      className={state =>
        cx(
          'focus-visible:ring-border-focus-ring outline-none focus-visible:rounded-sm focus-visible:ring-2',
          typeof className === 'function' ? className(state) : className,
        )
      }
    />
