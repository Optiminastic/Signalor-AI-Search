...
export function DropdownTrigger({ className, ...props }: ComponentProps<typeof AriaButton>) {
  ...
  return (
    <AriaButton
      ...
      {...props}
      className={cx(
        'focus-visible:ring-border-focus-ring cursor-pointer outline-none focus-visible:ring-2',
        className as string,
      )}
    />
  )
...
export function DropdownPopover({
  ...
  return (
    ...
    >
      <AriaDialog
        aria-label={ariaLabel}
        className={cx('flex flex-col outline-none', dialogClassName)}
      >
        {children}
      </AriaDialog>
    </AriaPopover>
...
export function DropdownDivider({ className }: { className?: string }) {
  return <div className={cx('bg-border-button-default -mx-2.5 my-2.5 h-px shrink-0', className)} />
}
